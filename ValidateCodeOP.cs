...
namespace ImageCode
{
        ...
        private Font[] fonts = 
    {
       ...
       new Font(new FontFamily("Comic Sans MS"), 10 + Next(1),System.Drawing.FontStyle.Regular)
        ...
        /// 
        /// </summary>
        /// <param name="len">一般4或5</param>
        /// <param name="lev">1-8</param>
        /// <param name="hei">一般20</param>
        public ValidateCodeOP(int len, int lev, int hei)
        {
            this.letterCount = len;
            this.letterHeight = hei;
            this.letterWidth = hei / 4 * 5;
            this.text = Rand.Number(len);
            CreateImage();
        }
        ...
        public void CreateImage()
        {
            ...
            g.Clear(Color.White);//整个将画布用白色填充
            //向画布里画银灰色随机线
            int count = 4;
            //根据lev值计算需要画的随机线的条数
            //...
            ...
            for (int int_index = 0; int_index < this.text.Length; int_index++)
            {
                ...
                str_char = Next(1) == 1 ? str_char.ToLower() : str_char.ToUpper();//随机改变大小写
        ...
        #region 快捷生成验证码 public static string Next(string filePath)
        /// <summary>
        /// 快捷生成验证码
        /// </summary>
        /// <param name="filePath">生成的图片的存放路径</param>
        /// <returns>生成的验证字符串</returns>
        public static string Next(string filePath)
        {
            ValidateCodeOP op = new ValidateCodeOP();
            op.Image.Save(filePath);
            return op.Text;
        } 
        #endregion
    }
    ...
    public class Test
    {
        ...
        {            
            Console.WriteLine(ValidateCodeOP.Next("c:\\valicode.png"));
            Console.ReadLine();
        }
    }
}
...
/** 验证码调用方法
 * string ValidateCodeOP.Next(string filePath);
 * srting ValiedateCodeOP.Next(string filePath,int len,int level,int hei);
**/